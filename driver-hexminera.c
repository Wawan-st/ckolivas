...
hexminera_send_task (struct hexminera_task *ht, struct cgpu_info *hexminera)
{
    ...
    //applog (LOG_ERR , "hexminera_create_task ret %0x", ret);
    if (ret != nr_len) {
...
static inline void
hexminera_create_task (bool reset_work, struct hexminera_task *ht, struct work *work)
{
    if (reset_work) {
    ...
    //applog (LOG_ERR , "hexminera_create_task ht->status %0x", ht->status );
...
hexminera_send_tasks (void *userdata)
{
    ...
    while(!libhexa_usb_dead(hexminera)) {
        ...
        while (info->read_pos < HEXMINERA_ARRAY_SIZE_REAL && info->hexworks[info->read_pos] != NULL &&
            ...
            hexminera_create_task (info->reset_work, &ht, info->hexworks[info->read_pos++]);
...
again:
          ...
          if (ret_r == 1 && wr->status < HEXA_STAT_UNUSED) {
            ...
            if(auto_times > HEXA_USB_TIMING_AUTO) {
              	...
              	if(a_val<HEXA_USB_TIMING_AJUST_LOW_RES) a_val = HEXA_USB_TIMING_AJUST_LOW_RES;
