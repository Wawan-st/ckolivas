  ...
  while (written < buf_len && err == LIBUSB_SUCCESS)
    {
      err = libusb_interrupt_transfer
        (usbdev->handle,
         0x01,
         sendbuf + written,
         MIN (HEXU_USB_WR_SIZE, buf_len - written), &wrote,
         timeout);
...
//Buggy on init but who cares :)
	...
	for (i = 0; i < 3; i++) {
		...
		na = p[i+16] + SHA_U[i] + h + Ch(e,f,g) + S1(e) + S0(a) + Maj(a,b,c);
		d = c; c = b; b = a; a = na;
		h = g; g = f; f = e; e = ne;
	}
...
static void 
libhexu_bitfury_payload_to_atrvec(uint32_t *atrvec, struct hexmineru_task *p)
{
...
static int
libhexu_bitfury_checkresults (struct thr_info *thr, struct work *work, uint32_t nonce)
{
  int i;
  for (i = 0; i < BT_OFFSETS_U; i++)
    {
      if (test_nonce (work, nonce + bf_offsetsu[i]))
        {
          submit_tested_work_no_clone(thr, work);
          return 1;
        }
    }
  return 0;
                           ...
                           uint32_t nonce, int work_id)
{
   ...
   mutex_unlock (&info->lock);
   res = libhexu_bitfury_checkresults (thr, work_sub, nonce); 
    
	 if(res == 0) free_work(work_sub);
out:
