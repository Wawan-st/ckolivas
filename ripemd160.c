...
static void ripemd160_transform(uint32_t *s, const uint8_t *chunk)
{
	...
	uint32_t w0  = le32toh(*(uint32_t*)(chunk +  0)), w1  = le32toh(*(uint32_t*)(chunk +  4)), w2  = le32toh(*(uint32_t*)(chunk +  8)), w3  = le32toh(*(uint32_t*)(chunk + 12));
	uint32_t w4  = le32toh(*(uint32_t*)(chunk + 16)), w5  = le32toh(*(uint32_t*)(chunk + 20)), w6  = le32toh(*(uint32_t*)(chunk + 24)), w7  = le32toh(*(uint32_t*)(chunk + 28));
	uint32_t w8  = le32toh(*(uint32_t*)(chunk + 32)), w9  = le32toh(*(uint32_t*)(chunk + 36)), w10 = le32toh(*(uint32_t*)(chunk + 40)), w11 = le32toh(*(uint32_t*)(chunk + 44));
	uint32_t w12 = le32toh(*(uint32_t*)(chunk + 48)), w13 = le32toh(*(uint32_t*)(chunk + 52)), w14 = le32toh(*(uint32_t*)(chunk + 56)), w15 = le32toh(*(uint32_t*)(chunk + 60));
...
{
	uint64_t size = htole64(ctx->pos * 8);
	ripemd160_update(ctx, pad, 1 + ((119 - (ctx->pos % 64)) % 64));
	ripemd160_update(ctx, (uint8_t*)&size, sizeof(size));

	*(uint32_t*)digest        = htole32(ctx->h[0]);
	*(uint32_t*)(digest +  4) = htole32(ctx->h[1]);
	*(uint32_t*)(digest +  8) = htole32(ctx->h[2]);
	*(uint32_t*)(digest + 12) = htole32(ctx->h[3]);
	*(uint32_t*)(digest + 16) = htole32(ctx->h[4]);
}
