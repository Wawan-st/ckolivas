...
static inline void
hexmineru_create_task (uint32_t *vec, struct hexmineru_task *ht, struct work *work)
{
...
hexmineru_read_write_tasks (void *userdata)
{
  ...
  libhexu_spi_emit_data(&spipos, (unsigned char *)wr_spi, 0x3000, &atrvec[0], 76);
  ...
  while (!libhexu_usb_dead(hexmineru))
    {
       ...
       cgsleep_prepare_r (&ts_start);
    	...
    	if(libhexu_nanofury_spi_txrx(hexmineru, &spipos, (unsigned char *)wr_spi, (unsigned char *)read_spi, false)) {
    		 ...
    		 //All below is stupid and slow but it works fine - three is no doc available for the moment how to make it faster.
    		 //Patches are welcomed!
					found = 0;
    		  for (i = 1; i < 17; i++) {
    		  	if (libhexu_cachenonce
             	 (&array_nonce_cache[0], read_spi[i])) {
             	 	  ...
             	 	  nonce = libhexu_decnonce (read_spi[i]); 
             			...
             			for (m = 0; m <= HEXMINERU_ARRAY_SIZE_REAL; m++) 	
											found+=hexmineru_predecode_nonce (hexmineru, thr, nonce, m);
             }   
          }
    	...
    	spipos = 80;
      cgsem_post(&info->qsem);
      if(!job_switch) cgsleep_us_r (&ts_start, 5*1000);
    }
...
*hexmineru_detect_one (libusb_device * dev, struct usb_find_devices *found)
{
  ...
  while (i < HEXMINERU_ARRAY_SIZE) info->hexworks[i++] = NULL; 
  ...
  if (!add_cgpu (hexmineru)) goto out;
  
  libhexu_reset (hexmineru);
  if(!libhexu_mcp2210_get_configs(hexmineru)) goto out;
  if(!hex_nanofury_checkport(hexmineru)) goto out;
  ...
  if(!libhexu_nanofury_spi_txrx(hexmineru, &spipos, (unsigned char *)buf, (unsigned char *)trash, true)) goto out;
...
hexmineru_shutdown (struct thr_info *thr)
{
  ...
  if (!hexmineru->shutdown) hexmineru->shutdown = true;
...
hexmineru_prepare (struct thr_info *thr)
{
 ...
 if (pthread_create
      (&info->write_thr, NULL, hexmineru_read_write_tasks, (void *) hexmineru))
...
static 
bool hexmineru_thread_init(struct thr_info *thr)
{
	struct cgpu_info *hexmineru = thr->cgpu;
	unsigned int wait;
